...
public:
    ...
    // Pass for simple evluation of the latency of the top function, without considering HLS directives
    HI_WithDirectiveTimingResourceEvaluation(//const char* config_file_name, 
                                             const HI_DesignConfigInfo &configInfo,
                                             const char* evaluating_log_name, 
                                             const char* BRAM_log_name,
                                             const char* ArrayLog_name, 
                                             const char* top_function, 
...
/////////////////////////////////////////////////////////////////////////////
    ...
    friend bool operator==(const DSPReuseScheduleUnit &lhs, const DSPReuseScheduleUnit &rhs)
    {
        ...
        {
            if (lhs.partitionBased != rhs.partitionBased)   return false;
            ...
            else
            {
                ...
                if (LInst && RInst)
                {
                    if (LInst->getOpcode() != RInst->getOpcode())  return false;
                    if (LInst->getType() != RInst->getType() )  return false;          
                }
                else
                {
                    if (lhs.LVal != lhs.LVal )  return false;  
                }
                ...
                {
                    if (lhs.RVal != lhs.RVal )  return false; 
                }

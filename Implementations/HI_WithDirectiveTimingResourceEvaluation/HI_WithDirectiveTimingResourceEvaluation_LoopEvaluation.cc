...
HI_WithDirectiveTimingResourceEvaluation::timingBase HI_WithDirectiveTimingResourceEvaluation::analyzeOuterLoop(Loop* outerL)
{
    ...
    {
        if (DEBUG) *Evaluating_log << "Done evaluation outer Loop Latency for Loop " << outerL->getName() << " and its latency is " << LoopLatency[outerL->getHeader()] <<" cycles.\n\n\n";
    ...
    while (cur_Loop!=NULL) 
    {
        ...
        if (II_for_loop <= -1 || II_for_loop >= max_critial_path_in_curLoop.latency)
        {
            ...
            if (cur_Loop->getSubLoops().size()>0 && isPerfectPipelinedNest(cur_Loop, innerHeader, total_nesttripcount))
            {
                ...
                if (DEBUG) *Evaluating_log << "This is a perfectly nested loop containing a pipelined subloop, we should flatten it.\n";
                if (DEBUG) *Evaluating_log << "total tripcount is " << total_nesttripcount
                                           << " inner pipeline II is " << Loop2AchievedII[innerHeader]
                                           << " inner iteration latency is " << Loop2IterationLatency[innerHeader] << "\n";
                tmp_total_latency = (total_nesttripcount-1) * Loop2AchievedII[innerHeader] * timingBase(1,0,1,clock_period) 
        ...
        else
        {
                ...
                && II_for_loop > 10)
            {
                ...
                if (DEBUG) *Evaluating_log << "the original trip count is " << original_tripcount << "\n";
        ...
        if (DEBUG) *Evaluating_log << "Done evaluation Loop Latency for Loop " << cur_Loop->getName() <<"(" <<  IRLoop2LoopLabel[tmp_loop_name] <<")" << " and its latency is " << tmp_total_latency <<" cycles and its resource cost is: " << LoopResource[cur_Loop->getHeader()] << ".\n\n\n";
    ...
    if (DEBUG) *Evaluating_log << "Done evaluation outer Loop Latency for Loop " << outerL->getName() << " and its latency is " << outerL_latency <<" cycles and its resource cost is: " << LoopResource[outerL->getHeader()] << ".\n\n\n";
...
void HI_WithDirectiveTimingResourceEvaluation::LoopLatencyResourceEvaluation_traversFromHeaderToExitingBlocks(HI_WithDirectiveTimingResourceEvaluation::timingBase tmp_critical_path, Loop* L, BasicBlock *curBlock, HI_WithDirectiveTimingResourceEvaluation::resourceBase &resourceAccumulator)
{
    ...
    {        
        ...
        if (checkFlag)
        {
            if (tmp_SubLoop_CriticalPath.find(tmp_SubLoop) != tmp_SubLoop_CriticalPath.end())
            {
                if (DEBUG) *Evaluating_log << " OriCP =  " << tmp_SubLoop_CriticalPath[tmp_SubLoop] <<"\n";
            }
            else
            {
    ...
    else
    {
        ...
        if (DEBUG) *Evaluating_log << "---- latencyBlock =  " << latency_CurBlock <<"    tmp_critical_path_in_loop=" << tmp_critical_path << "  ";
        if (DEBUG) *Evaluating_log << " NewCP =  " << try_critical_path <<" ";
        bool checkFlag = false;
        ...
        if (checkFlag) // update the block-level critical path
        {
            if (tmp_BlockCriticalPath_inLoop.find(curBlock) != tmp_BlockCriticalPath_inLoop.end())
            {
                if (DEBUG) *Evaluating_log << " OriCP =  " << tmp_BlockCriticalPath_inLoop[curBlock] <<"\n";
            }
            else
            {
                if (DEBUG) *Evaluating_log << " No OriCP" <<"\n";
...
int HI_WithDirectiveTimingResourceEvaluation::checkIIForLoop(Loop *curLoop,  std::map<BasicBlock*, timingBase> &tmp_BlockCriticalPath_inLoop)
{
   ...
   int II_BRAM_enum = checkAccessIIForLoop_enumerateCheck(curLoop);
    ...
    if (II_BRAM > min_II)
    {
        ...
        if (DEBUG) *Evaluating_log << "--------- Loop pipeline cannot achieve expected II because of BRAM limitation\n";
    }
...
int HI_WithDirectiveTimingResourceEvaluation::checkAccessIIForLoop(Loop* curLoop)
{
    ...
    for (auto tmp_B : curLoop->getBlocks())
    {
        ...
        for (auto it_value_partition_2_cnt : accessCounterForBlock[tmp_B] )
        {
            ...
            {
                if (DEBUG) *Evaluating_log << "--------- min II is updated to " << (accessTotalCntInLoop+partID.port_num-1)/partID.port_num << " because\n";
...
int HI_WithDirectiveTimingResourceEvaluation::checkDependenceIIForLoop(Loop* curLoop)
{
    ...
    for (auto InstInst2DependenceDistance_pair : InstInst2DependenceDistance)
    {
        ...
        if (tmp_II > min_II)
        {
            ...
            if (DEBUG) *ArrayLog << "      W_I_time_offset=" << W_I_time_offset << " R_I_time_offset=" << R_I_time_offset << " DependenceDistance=" << InstInst2DependenceDistance_pair.second << "\n";
        }
    }
        ...
        for (auto &I : *tmp_B)
        {
            if (auto PHI_I = dyn_cast<PHINode>(&I))
            {
                for (int i=0; i<PHI_I->getNumIncomingValues();i++)
                {
                    ...
                    {
                        if (DEBUG) *ArrayLog << "      PHI Feedback dependence between " << *PHI_I << "  and " << *tmp_I << "\n";
                        ...
                        if (DEBUG) *ArrayLog << "      W_I_time_offset=" << W_I_time_offset << "(" << BlockBegin_inLoop[tmp_I->getParent()].latency 
...
void HI_WithDirectiveTimingResourceEvaluation::checkLoopCarriedDependent(Instruction *I0, Instruction *I1, Loop* curLoop)
{
    ...
    {        
        ...
        else if (offset_dis == 0)
        {
            if (DEBUG)
            {
                *ArrayLog << "ready to checkConstantAccessInLoop: I0=" << *I0 
                        << "   I1=" << *I1 << "\n";
            ...
            else
            {
                ...
                InstInst2DependenceDistance[std::pair<Instruction*, Instruction*>(I0,I1)] = 1;
                if (DEBUG) *ArrayLog << "**** distance:" << *I0 << " <=> " << *I1 << " =" 
                            << 1 << " offset_dis=" << offset_dis << " stepLen=" << stepLen << " but constant in loop II should be greater than 1.\n";
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::costRescheduleIntDSPOperators_forLoop(Loop *curLoop,  std::map<BasicBlock*, timingBase> &tmp_BlockCriticalPath_inLoop, int II)
{
    ...
    for (auto tmp_block : curLoop->getBlocks())
    {
        ...
        for (auto tmp_schUnit0 : Block2IntDSPReuseScheduleUnits[tmp_block])
        {
            ...
            if (DEBUG) *Evaluating_log << "max_originalConflictCnt=" << max_originalConflictCnt << "  max_newConflictCnt=" << max_newConflictCnt << "\n";
            if (DEBUG) *Evaluating_log << "increased resource=" <<  getInstructionResource(tmp_schUnit0.opI)*(max_newConflictCnt - max_originalConflictCnt) << "\n";
...
void HI_WithDirectiveTimingResourceEvaluation::recordCostRescheduleFPDSPOperators_forLoop(Loop *curLoop,  std::map<BasicBlock*, timingBase> &tmp_BlockCriticalPath_inLoop, int II)
{
    ...
    for (auto cur_opcode : checkopcodes)
    {
        ...
        if (op_totalcnt > II) reuse_DSPModule = (op_totalcnt + II - 1) / II;
...
bool HI_WithDirectiveTimingResourceEvaluation::isPerfectPipelinedNest(Loop *L, BasicBlock *&innerHeader, int &total_nesttripcount)
{
	...
	else
	{
        ...
        {
            if (DEBUG) *Evaluating_log << "  curLoop:" << L->getName() << " has more than one sub-loops:\n";
            for (auto tmpL : L->getSubLoops())
            {
                if (DEBUG) *Evaluating_log << "    subLoop:" << tmpL->getName() << "\n";
            }
			return false;
        }

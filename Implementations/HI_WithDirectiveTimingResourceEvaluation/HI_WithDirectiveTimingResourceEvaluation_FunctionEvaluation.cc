...
*/
HI_WithDirectiveTimingResourceEvaluation::timingBase HI_WithDirectiveTimingResourceEvaluation::analyzeFunction(Function *F) 
{
    ...
    // analyze the cost of floating-point operators (most of them will be aggressive reused by VivadoHLS binding)
    ...
    if (DEBUG) *Evaluating_log << "---- Function " << F->getName() << " includes "<<Function2OuterLoops[F].size()<<" following most outer loop(s):\n-------";
    for (auto tmpL : Function2OuterLoops[F])
    {
        if (DEBUG) *Evaluating_log << " Loop:" << tmpL->getName() << "(lat=" << LoopLatency[tmpL->getHeader()] << ") ";
    }
    if (DEBUG) *Evaluating_log << "\n---- Function " << F->getName() << " includes following block(s) out of loops:\n-------";
    for (auto &B_it : *F)
    {
        if (!isInLoop(&B_it))
            if (DEBUG) *Evaluating_log << " Block:" << B_it.getName() << "(lat=" << BlockLatency[&B_it] << ") ";
    }
    if (DEBUG) *Evaluating_log << "\n\n\n\n";
...
/*
    ...
    (3a) -- If it is a block in loops, regard the most outer loop as intergration and update the critical path if necessary (max(ori_CP, lastStateCP + LoopLatency)). 
...
*/
void HI_WithDirectiveTimingResourceEvaluation::analyzeFunction_traverseFromEntryToExiting(HI_WithDirectiveTimingResourceEvaluation::timingBase tmp_critical_path, Function *F, BasicBlock* curBlock, HI_WithDirectiveTimingResourceEvaluation::resourceBase &resourceAccumulator)
{
    ...
    if (isInLoop(curBlock)) 
    {
        ...
        if (DEBUG) *Evaluating_log << "---- Block: " << curBlock->getName() <<" is in Outer Loop : "<< tmp_OuterLoop->getName() <<" ";        
        ...
        if (checkFlag)
        {
            if (tmp_LoopCriticalPath_inFunc.find(tmp_OuterLoop) != tmp_LoopCriticalPath_inFunc.end())
            {
                if (DEBUG) *Evaluating_log << " OriCP =  " << tmp_LoopCriticalPath_inFunc[tmp_OuterLoop] <<"\n";                
            }
            else
            {
            ...
            for (auto ExitB : tmp_OuterLoop_ExitingBlocks)
            {
                for (auto B : successors(ExitB))
                {
                    ...
                    {
                        if (DEBUG) *Evaluating_log << "---- function continue to traverser to Block: " << B->getName()  << " from " << curBlock->getName() <<"\n";
    ...
    {
        //     (3b) -- If it is a block out of loops, evaluate the block latency and update the critical path if necessary (max(ori_CP, lastStateCP + BlockLatency)).         
        if (DEBUG) *Evaluating_log << "---- Block: " << curBlock->getName() <<" is NOT in any Outer Loop ";
        ...
        if (checkFlag) // update the block-level critical path
        {
            if (tmp_BlockCriticalPath_inFunc.find(curBlock) != tmp_BlockCriticalPath_inFunc.end())
            {
                if (DEBUG) *Evaluating_log << " OriCP =  " << tmp_BlockCriticalPath_inFunc[curBlock] <<"\n";
            }
            else
            {
                if (DEBUG) *Evaluating_log << " No OriCP" <<"\n";
...
int HI_WithDirectiveTimingResourceEvaluation::getFunctionStageNum(HI_WithDirectiveTimingResourceEvaluation::timingBase tmp_critical_path, Function *F, BasicBlock* curBlock)
{
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::getFunctionResource(Function *F)
{
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::costRescheduleFPDSPOperators_forFunction(Function* F)
{
    ...
    for (auto cur_opcode : checkopcodes)
    {
        ...
        if (DEBUG) *Evaluating_log << "  for function: the amount of floating point operators (refI):" << cur_opcode << " is " << max_cnt << " each cost =[" << checkFPOperatorCost(cur_opcode) << "]\n";
...
void HI_WithDirectiveTimingResourceEvaluation::functionDataflowCheck(Function* F)
{
    ...
    // there should be any loop in the function (original loops should be extracted into sub-functions)
    for (auto &B : *F)
    {
        if (isInLoop(&B)) 
        {
            llvm::errs() << "DATAFLOW ERROR: loop in function: [" << functionDemangleName << "]\n";
            assert(false && "there should not be any loop in the function (original loops should be extracted into sub-functions)");
        }
    }
        ...
        for (auto &I : B)
        {
            if (auto callI = dyn_cast<CallInst>(&I))
            {
                ...
                else
                {
                    if (&B != blockforcallIs)
                    {
                        llvm::errs() << "DATAFLOW ERROR: call instruction in branches in function: [" << functionDemangleName << "]\n";
                        assert(false && "there should not be any call instruction in different branches in the function (original loops should be extracted into sub-functions)");
                    }
                }
    ...
    *Evaluating_log << " Function: [" << functionDemangleName << "] is applied with Dataflow, checking its II\n" ;
    ...
    *Evaluating_log << " Function: [" << functionDemangleName << "] is applied with Dataflow, checked its II=" << II_dataflow << "\n" ;

    *ArrayLog << " Function: [" << functionDemangleName << "] is applied with Dataflow, checking its BRAM cost\n" ;
        ...
        for (auto tmp_target : stage)
        {
            ...
            else
            {
                if (array2LastStage[tmp_target] != stage_cnt) 
                {
                    ...
                    if (array2LastStage[tmp_target]==0 && targetAccessPropertyInFunction[std::pair<Value*, Function*>(tmp_target, F)]==WriteOnlyInFunction)
                    {
                        crossStagesNum = stage2arrays.size() - stage_cnt; // this array is for output. Therefore, it should be buffered to the end of dataflow
                    ...
                    else
                    {
                        ...
                        // if the array is write-only, it does not need the data from previous stages
                        if (targetAccessPropertyInFunction[std::pair<Value*, Function*>(tmp_target, callI_curStage->getCalledFunction())]!=WriteOnlyInFunction)
                            crossStagesNum = stage_cnt - array2LastStage[tmp_target]; // this array needs the output from other stages. 
                        // WARNING: currently, we ignore the constraint of one-producer-one-consumer. We assume that the source code is designed according to this constraint.
                    }                   
                    
                    *ArrayLog << " Function: [" << functionDemangleName << "] is applied with Dataflow, array: [" << tmp_target->getName() << "] need buffer BRAMs.\n" ;
                    *ArrayLog << "      The buffer should bridge [" << crossStagesNum << "] stages\n" ;
                    ...
                    if (auto allocI = dyn_cast<AllocaInst>(tmp_target))
                        ...
                        assert(false && "the array should be generated from arguments or allocation instruction.\n");
                    *ArrayLog << "      each buffer for the array will cost resourse: [" <<  targetBRAMcost << "] \n";
                    *ArrayLog << "      all buffers for the array will cost resourse: [" <<  2*crossStagesNum*targetBRAMcost << "] \n";

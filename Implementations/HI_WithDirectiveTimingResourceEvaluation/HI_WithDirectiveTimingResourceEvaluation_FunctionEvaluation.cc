        ...
        if (checkFlag)
        {
            ...
            for (auto ExitB : tmp_OuterLoop_ExitingBlocks)
            {
                for (auto B : successors(ExitB))
                {
                    ...
                    {
                        if (DEBUG)
                            *Evaluating_log << "---- function continue to traverser to Block: " << B->getName() << " from " << curBlock->getName() << "\n";
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::costRescheduleFPDSPOperators_forFunction(Function *F)
{
    ...
    for (auto cur_opcode : checkopcodes)
    {
        ...
        if (DEBUG)
            *Evaluating_log << "  for function: the amount of floating point operators (refI):" << cur_opcode << " is " << max_cnt << " each cost =[" << checkFPOperatorCost(cur_opcode) << "]\n";

...
HI_WithDirectiveTimingResourceEvaluation::timingBase HI_WithDirectiveTimingResourceEvaluation::scheduleBRAMAccess(Instruction *access, BasicBlock *cur_block, HI_WithDirectiveTimingResourceEvaluation::timingBase cur_Timing, partition_info target_partition)
{
    ...
    if (Target2ArrayInfo[target].completePartition)
    {
        ...
        {
            if (DEBUG)
                *BRAM_log << " sceduled access Store instruction: " << *access << " for the target [" << target->getName() << "] in its partition #" << target_partition << " at cycle #" << targetTiming.latency << " of Block:" << cur_block->getName() << "\n";
        }
        else
        {
            if (DEBUG)
                *BRAM_log << " sceduled access Load instruction: " << *access << " for the target [" << target->getName() << "] in its partition #" << target_partition << " at cycle #" << targetTiming.latency << " of Block:" << cur_block->getName() << "\n";
        }
    ...
    else
    {
        ...
        {
            if (DEBUG)
                *BRAM_log << " sceduled access Load instruction: " << *access << " for the target [" << target->getName() << "] in its partition #" << target_partition << " at cycle #" << targetTiming.latency - 1 << " of Block:" << cur_block->getName() << "\n";
        }
...
HI_WithDirectiveTimingResourceEvaluation::timingBase HI_WithDirectiveTimingResourceEvaluation::handleBRAMAccessFor(Instruction *access, Value *target, BasicBlock *cur_block, HI_WithDirectiveTimingResourceEvaluation::timingBase cur_Timing, partition_info target_partition)
{
...
bool HI_WithDirectiveTimingResourceEvaluation::checkBRAMAvailabilty(Instruction *access, Value *target, std::string StoreOrLoad, BasicBlock *cur_block, HI_WithDirectiveTimingResourceEvaluation::timingBase cur_Timing, partition_info target_partition)
{
    if (DEBUG)
...
{
    if (DEBUG)
        *BRAM_log << "       inserting the access to the target [" << target->getName() << "] in its partition #" << target_partition << " in cycle #" << cur_latency << " of Block:" << cur_block->getName() << "  --> ";
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::get_BRAM_Num_For(AllocaInst *alloca_I)
{
    ...
    if (DEBUG)
        *BRAM_log << "checked allocation instruction [" << *alloca_I << "] and its basic elemenet type is: [" << *tmp_type << "] with BW=[" << BW << "] and the total number of basic elements of each partition is: [" << eachPartitionDepth << "] and the total number of partitions is " << totalPartitionNum << "and it need BRAMs [" << res.BRAM << "].\n\n";
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::get_BRAM_Num_For(Argument *ArgTarget)
{
    ...
    if (DEBUG)
        *BRAM_log << "checked argument [" << *ArgTarget << "] and its basic elemenet type is: [" << *tmp_type << "] with BW=[" << BW << "] and the total number of basic elements of each partition is: [" << eachPartitionDepth << "] and the total number of partitions is " << totalPartitionNum << "and it need BRAMs [" << res.BRAM << "].\n\n";
...
void HI_WithDirectiveTimingResourceEvaluation::TryArrayAccessProcess(Instruction *I, ScalarEvolution *SE)
{
    ...
    */
    ...
    if (getUnknownNum(tmp_S) > 1)
    {
        ...
        // in the array
        if (DEBUG)
            *ArrayLog << *I << " --> SCEV: " << *tmp_S << " has multiple unknown values in expression. It will be a complex access.\n";
...
HI_WithDirectiveTimingResourceEvaluation::HI_AccessInfo HI_WithDirectiveTimingResourceEvaluation::getAccessInfoFor(Value *target, Instruction *access, int initial_offset, std::vector<int> *inc_indices, std::vector<int> *trip_counts, bool unpredictable)
{
...
std::vector<HI_WithDirectiveTimingResourceEvaluation::partition_info> HI_WithDirectiveTimingResourceEvaluation::getPartitionFor(Instruction *access)
{
    ...
    else
    {
        ...
        if (partitions.size() == 1)
        {
            Access2EnableArrayNo[access] = 0;
            if (DEBUG)
                *ArrayLog << "addressCalculationCheck: the access Inst: " << *access << " access  one constant partition.\n";
        }
        else
        {
            for (int i = 0; i < tmp_res.num_dims; i++)
            {
                ...
                {
                    if (DEBUG)
                        *ArrayLog << "addressCalculationCheck: the access Inst: " << *access << " partition@dim=" << i + 1 << " 2-power .\n";
                }
            }
        }
    }
    ...
    {
        if (DEBUG)
            *ArrayLog << "addressCalculationCheck: the access Inst: " << *access << " requires remainder calculation.\n";
    }
    else
    {
        if (DEBUG)
            *ArrayLog << "addressCalculationCheck: the access Inst: " << *access << " does not require remainder calculation.\n";
    }
...
AliasResult HI_WithDirectiveTimingResourceEvaluation::HI_AAResult::alias(const MemoryLocation &LocA, const MemoryLocation &LocB)
{
    //   auto PtrA = LocA.Ptr;
    //   auto PtrB = LocB.Ptr;
        ...
        if (auto IPT = dyn_cast<IntToPtrInst>(pointer_I))
        {
            if (auto PTI = dyn_cast<PtrToIntInst>(IPT->getOperand(0)))
            {
                ...
                if (auto arg_pointer = dyn_cast<Argument>(target))
                {
                    ...
                    if (DEBUG)
                        *ArrayLog << " -----> access info with array index: " << AddressInst2AccessInfo[pointer_I->getOperand(0)] << "\n\n\n";
                    // ArrayLog->flush();
                }
                else if (auto alloc_pointer = dyn_cast<AllocaInst>(target))
                {
...
bool HI_WithDirectiveTimingResourceEvaluation::checkConstantAccessInLoop(const SCEV *ori_More, const SCEV *ori_Less, Loop *curLoop)
{
    // We avoid subtracting expressions here because this function is usually
    // fairly deep in the call stack (i.e. is called many times).
    if (DEBUG)
    {
        *ArrayLog << "checkConstantAccessInLoop: ori_More=" << *ori_More << "   ori_Less=" << *ori_Less << " curLoop:" << curLoop->getName() << "\n";
        ArrayLog->flush();
    }
...
bool HI_WithDirectiveTimingResourceEvaluation::splitBinaryAdd(const SCEV *Expr, const SCEV *&L, const SCEV *&R, SCEV::NoWrapFlags &Flags)
{
...
void HI_WithDirectiveTimingResourceEvaluation::checkPartitionBenefit(std::vector<HI_WithDirectiveTimingResourceEvaluation::partition_info> &partitions, Instruction *accessI)
{
    ...
    if (num_partitions != num_potential_target_partitions)
    {
        ...
        if (DEBUG)
        {
            *ArrayLog << "INFO: partitioning helps to improve the parallelism for the accesses: " << *accessI << "(target=" << *target_array << ")\n\n";
        }
    }
}
...
HI_WithDirectiveTimingResourceEvaluation::resourceBase HI_WithDirectiveTimingResourceEvaluation::BRAMRelatedCostForTopFunction(Function &F)
{
    ...
    for (auto it = F.arg_begin(), ie = F.arg_end(); it != ie; ++it)
    {
        if (it->getType()->isPointerTy())
        {
            ...
            {
                if (DEBUG)
                    *Evaluating_log << "  get array information of [" << it->getName() << "] from argument and its address=" << it << "\n";

...
bool HI_ConstantOffsetExtractor::CanTraceInto(bool SignExtended,
                                            bool ZeroExtended,
                                            BinaryOperator *BO,
                                            bool NonNegative) {
...
APInt HI_ConstantOffsetExtractor::findInEitherOperand(BinaryOperator *BO,
                                                   bool SignExtended,
                                                   bool ZeroExtended) {
...
APInt HI_ConstantOffsetExtractor::find(Value *V, bool SignExtended,
...
Value *
HI_ConstantOffsetExtractor::distributeExtsAndCloneChain(unsigned ChainIndex) {
...
Value *HI_ConstantOffsetExtractor::Extract(Value *Idx, GetElementPtrInst *GEP,
                                        User *&UserChainTail,
...
bool HI_SeparateConstOffsetFromGEP::canonicalizeArrayIndicesToPointerSize(
...
int64_t
HI_SeparateConstOffsetFromGEP::accumulateByteOffset(GetElementPtrInst *GEP,
  ...
  for (unsigned I = 1, E = GEP->getNumOperands(); I != E; ++I, ++GTI) {
    if (GTI.isSequential()) {
      ...
      int64_t ConstantOffset =
          HI_ConstantOffsetExtractor::Find(GEP->getOperand(I), GEP, DT);
      if (ConstantOffset != 0) {
        ...
        AccumulativeByteOffset +=
            ConstantOffset * getLength(GTI.getIndexedType());                      //HI-MODIFICATION: In HLS, data are not stored in bytes.
...
void HI_SeparateConstOffsetFromGEP::lowerToSingleIndexGEPs(
  ...
  for (unsigned I = 1, E = Variadic->getNumOperands(); I != E; ++I, ++GTI) {
    if (GTI.isSequential()) {
      ...
      APInt ElementSize = APInt(IntPtrTy->getIntegerBitWidth(),//1);   //HI-MODIFICATION: we don't need to calculate by byte in HLS
...
void
HI_SeparateConstOffsetFromGEP::lowerToArithmetics(GetElementPtrInst *Variadic,
 ...
 //   return Changed;   
  ...
  for (unsigned I = 1, E = GEP->getNumOperands(); I != E; ++I, ++GTI) {
    if (GTI.isSequential()) {
      ...
      User *UserChainTail;
      Value *NewIdx =
          HI_ConstantOffsetExtractor::Extract(OldIdx, GEP, UserChainTail, DT);
      if (NewIdx != nullptr) {
  ...
  int64_t ElementTypeSizeOfGEP = static_cast<int64_t>(//1);     //HI-MODIFICATION: we don't need to calculate by byte in HLS
...
Instruction *HI_SeparateConstOffsetFromGEP::findClosestMatchingDominator(
...
bool HI_SeparateConstOffsetFromGEP::isLegalToSwapOperand(

...
bool HI_ReplaceSelectAccess::runOnFunction(Function &F) // The runOnModule declaration will overide the virtual one in ModulePass, which will be executed for each Module.
{
    ...
    for (auto &B : F)
    {
        ...
        while (ActionTaken)
        {
            ...
            for (auto &I : B)
            {
                if (DEBUG) *ReplaceSelectAccess_Log << "checking instruction: " <<  I  << "\n";
                if (auto selectI = dyn_cast<SelectInst>(&I))
                {
                    if (DEBUG) *ReplaceSelectAccess_Log << " is selectInst\n";
                    if (checkUseInMemoryAccess(&I) && checkOperandsAreAddress(&I))
                    {
                        if (DEBUG) *ReplaceSelectAccess_Log << "\n need to be processed\n";
                        ActionTaken = true;
...
bool HI_ReplaceSelectAccess::checkOperandsAreAddress(Instruction *I)
{
    for (int i=1; i<I->getNumOperands(); i++)
    {
        ...
        {
            if (DEBUG) *ReplaceSelectAccess_Log << "op#" << i << ": " << *(I->getOperand(i)) << " is ITP\n";
        }
        else
        {
            if (DEBUG) *ReplaceSelectAccess_Log << "op#" << i << ": " << *(I->getOperand(i)) << " is NOT ITP\n";
            return false;
        }
...
bool HI_ReplaceSelectAccess::checkUseInMemoryAccess(Instruction *I)
{
    ...
    {
        if (DEBUG) *ReplaceSelectAccess_Log << *(I) << " is used for Load\n";
        return true;
    }
    else if (auto StoreI = dyn_cast<StoreInst>(tmp_user))
    {
        if (DEBUG) *ReplaceSelectAccess_Log << *(I) << " is used for Store\n";
        return true;
    }
    else
    {
...
{
    if (DEBUG) *ReplaceSelectAccess_Log << *(I) << " is being From_SelectAccess_To_AccessSelect\n";
    ...
    std::string op0str = I->getName(); op0str+=".access0";
    std::string op1str = I->getName(); op1str+=".access1";
    std::string selectNewStr = selectI->getName(); selectNewStr+=".accessSelect";
    ...
    {                                                
        ...
        if (DEBUG) *ReplaceSelectAccess_Log << "newop0:" << *op0 << "\n";
        if (DEBUG) *ReplaceSelectAccess_Log << "newop1:" << *op1 << "\n";
        Value* newSelect = Builder.CreateSelect(selectI->getOperand(0), op0, op1, selectNewStr.c_str());
        if (DEBUG) *ReplaceSelectAccess_Log << "newSelect:" << *newSelect << "\n";

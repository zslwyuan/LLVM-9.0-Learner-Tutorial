...
HI_NoDirectiveTimingResourceEvaluation::analyzeOuterLoop(Loop *outerL)
{
...
*/
void HI_NoDirectiveTimingResourceEvaluation::
    LoopLatencyResourceEvaluation_traversFromHeaderToExitingBlocks(
        HI_NoDirectiveTimingResourceEvaluation::timingBase tmp_critical_path, Loop *L,
        BasicBlock *curBlock,
        HI_NoDirectiveTimingResourceEvaluation::resourceBase &resourceAccumulator)
{
        ...
        *Evaluating_log << " NewCP =  " << try_critical_path << " ";
        ...
        if (checkFlag) // update the block-level critical path
        {
            ...
            for (auto B : successors(curBlock))
            {
                ...
                {
                    *Evaluating_log
                        << "---- continue to traverser from Block: " << curBlock->getName()
                        << " to Block: " << B->getName() << " ";
                    LoopLatencyResourceEvaluation_traversFromHeaderToExitingBlocks(
                        try_critical_path, L, B, resourceAccumulator);
                }
...
*/
void HI_NoDirectiveTimingResourceEvaluation::MarkBlock_traversFromHeaderToExitingBlocks(
    HI_NoDirectiveTimingResourceEvaluation::timingBase total_latency, Loop *L, BasicBlock *curBlock)
{
    BlockVisited.insert(curBlock);

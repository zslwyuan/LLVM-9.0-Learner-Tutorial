...
HI_ArraySensitiveToLoopLevel::resourceBase HI_ArraySensitiveToLoopLevel::get_BRAM_Num_For(AllocaInst *alloca_I)
{
    ...
    if (DEBUG) *BRAM_log << "checked allocation instruction [" << *alloca_I << "] and its basic elemenet type is: [" << *tmp_type << "] with BW=[" <<BW << "] and the total number of basic elements is: [" << total_ele << "] and it need BRAMs [" << res.BRAM << "].\n\n";
...
HI_ArraySensitiveToLoopLevel::HI_AccessInfo HI_ArraySensitiveToLoopLevel::getAccessInfoFor(Value* target, 
...
void HI_ArraySensitiveToLoopLevel::findTheIncrementalIndexAndTripCount(Value *target, const SCEVAddRecExpr *S, std::vector<int> &inc_indices, std::vector<int> &trip_counts)
{
    ...
    if (DEBUG) *ArrayLog << "we are checking loop: " << IRLoop2LoopLabel[tmp_loop_name]  << " (tmp_loop_name=" << tmp_loop_name << ")\n" ;
    if (DEBUG) *ArrayLog << "its inc_const=" << inc_const  << "\n" ;
    if (DEBUG) *ArrayLog << "refArray=" << refArrayInfo  << "\n" ;
    for (int i=0; i < refArrayInfo.num_dims; i++)
    {
        if (DEBUG) *ArrayLog << " refArrayInfo.sub_element_num[i+1]=" << refArrayInfo.sub_element_num[i] * refArrayInfo.dim_size[i]
        ...
        if (inc_const < refArrayInfo.sub_element_num[i] * refArrayInfo.dim_size[i] && inc_const >= refArrayInfo.sub_element_num[i])
        {
            if (inc_const % refArrayInfo.sub_element_num[i] == 0)
            {
                ...
                if (DEBUG) *ArrayLog << "loop: " << IRLoop2LoopLabel[tmp_loop_name] 
                                     << " should drive the partitioning of dimension#" << (i+1) 
                                     << "(WARNING reverse order) of Array: [" << target->getName() << "] in function: ["
                                     << targetF->getName() << "]\n";

                std::pair<std::string, std::pair<std::string, int>> info_triple 
                            = std::pair<std::string, std::pair<std::string, int>>(targetF->getName(), 
                                                      std::pair<std::string, int>(target->getName(), i+1));
...
int HI_ArraySensitiveToLoopLevel::getStepLength(const SCEV *ori_More,
                                                            const SCEV *ori_Less) 
{
    // We avoid subtracting expressions here because this function is usually
    // fairly deep in the call stack (i.e. is called many times).
...
bool HI_ArraySensitiveToLoopLevel::splitBinaryAdd(const SCEV *Expr,

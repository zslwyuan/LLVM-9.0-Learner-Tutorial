...
pathHead = os.path.abspath(sys.argv[0]).replace('microTest_sitofp.py',"")
...
float top(ap_int<YYYYY> a, ap_int<YYYYY> b)
{
#pragma HLS pipeline
...
tcl_source_code = """
open_project microBench_sitofp
set_top top
add_files FFFFF
...
if (len(re.findall(r"^sitofp_[0-9]+_[0-9]+_",input_settings))>0):
    paras = input_settings.replace("sitofp_","").split("_")

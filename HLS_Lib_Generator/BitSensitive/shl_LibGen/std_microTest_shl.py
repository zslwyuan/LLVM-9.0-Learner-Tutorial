...
pathHead = os.path.abspath(sys.argv[0]).replace('microTest_shl.py',"")
...
if (len(re.findall(r"^shl_[0-9]+_[0-9]+_",input_settings))>0):
    paras = input_settings.replace("shl_","").split("_")
...
for oprandC in range(startBitwidth, endBitwidth+1):
    ...
    for clock_rate in [3,4,5,6,7,8,9,10,12.5,15,16,17.5,20,25,30]:
        ...
        # delay
        for line in report_content:
            if (line.find(":1")>=0 and line.find("\'shl\'")>=0):

...
pathHead = os.path.abspath(sys.argv[0]).replace('microTest_fptoui.py',"")
...
ap_uint<YYYYY> top(float a, ap_int<YYYYY> b)
{
#pragma HLS pipeline
...
tcl_source_code = """
open_project microBench_fptoui
set_top top
add_files FFFFF
...
if (len(re.findall(r"^fptoui_[0-9]+_[0-9]+_",input_settings))>0):
    paras = input_settings.replace("fptoui_","").split("_")

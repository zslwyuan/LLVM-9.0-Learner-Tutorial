...
pathHead = os.path.abspath(sys.argv[0]).replace('microTest_sdiv.py',"")
...
if (len(re.findall(r"^sdiv_[0-9]+_[0-9]+_",input_settings))>0):
    paras = input_settings.replace("sdiv_","").split("_")
...
for oprandC in range(startBitwidth, endBitwidth+1):
    ...
    for clock_rate in [3,4,5,6,7,8,9,10,12.5,15,16,17.5,20,25,30]:
        ...
        start_check = 0
        for line in report_content:
            if (line.find("|   sdiv ")>=0 and start_check==1):
        ...
        # delay
        for line in report_content:
            if (line.find(":1")>=0 and line.find("\'sdiv\'")>=0):

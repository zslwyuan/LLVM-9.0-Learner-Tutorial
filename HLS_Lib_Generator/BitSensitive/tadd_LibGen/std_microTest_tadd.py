...
pathHead = os.path.abspath(sys.argv[0]).replace('microTest_tadd.py',"")
...
ap_int<ZZZZZ> top(ap_int<XXXXX> a, ap_int<YYYYY> b, ap_int<YYYYY> e)
{
...
if (len(re.findall(r"^tadd_[0-9]+_[0-9]+_",input_settings))>0):
    paras = input_settings.replace("tadd_","").split("_")
...
for oprandC in range(startBitwidth, endBitwidth+1):
    ...
    for clock_rate in [3,4,5,6,7,8,9,10,12.5,15,16,17.5,20,25,30]:
        ...
        for line in report_content:
            if (line.find("root node of TernaryAdder")>=0 ):
                noTadd = 0
        
        # not all the device support ternary adder tree.
        if (noTadd):
            continue
        ...
        # delay
        for line in report_content:
            if (line.find(":1")>=0 and line.find("root node of TernaryAdder")>=0):
